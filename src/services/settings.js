...
/**
 * Fetches all registration categories
 *
 * @returns {Promise<Array>} Array of registration categories
 */
export async function getRegistrationCategories() {
  try {
    const categoriesRef = collection(db, COLLECTIONS.CONFERENCES, SETTINGS_DOC_ID, 'registrationCategories');
    const categoriesQuery = query(categoriesRef, orderBy('displayOrder', 'asc'));
    ...
    console.error('Failed to fetch registration categories:', error);
    return [];
  }
}

/**
 * Gets active registration categories for public registration
 * Excludes admin-only categories
 *
 * @returns {Promise<Array>} Array of active public registration categories
 */
export async function getActiveRegistrationCategories() {
  try {
    const categories = await getRegistrationCategories();
    return categories.filter((category) => category.isActive && !category.isAdminOnly);
  } catch (error) {
    console.error('Failed to fetch active registration categories:', error);
    return [];
  }
}
 ...
 *
 * @param {Object} category - Registration category data
 * @param {string} category.name - Display name (e.g., "Regular", "VIP")
 * @param {string} category.key - Unique identifier (e.g., "regular", "vip")
 * @param {number} category.price - Price for this category
 * @param {string} category.description - Category description
 * @param {boolean} category.isActive - Whether category is active
 * @param {boolean} category.isAdminOnly - Whether only admins can assign
 * @param {number} category.displayOrder - Sort order
 * @param {string} adminId - Admin user ID performing the action
 * @param {string} adminEmail - Admin email performing the action
 * @returns {Promise<Object>} Created category with ID
 */
export async function createRegistrationCategory(category, adminId = null, adminEmail = null) {
  try {
    const categoriesRef = collection(db, COLLECTIONS.CONFERENCES, SETTINGS_DOC_ID, 'registrationCategories');
    ...
    if (adminId && adminEmail) {
      await logActivity({
        ...
        entityId: docRef.id,
        description: `Created registration category: ${category.name}`,
        adminId,
        adminEmail,
      });
    }
    ...
    console.error('Failed to create registration category:', error);
    throw error;
  }
}

/**
 * Updates an existing registration category
 *
 ...
 */
export async function updateRegistrationCategory(categoryId, category, adminId = null, adminEmail = null) {
  try {
    const categoryRef = doc(db, COLLECTIONS.CONFERENCES, SETTINGS_DOC_ID, 'registrationCategories', categoryId);
    ...
    if (adminId && adminEmail) {
      await logActivity({
        ...
        entityType: ENTITY_TYPES.CATEGORY,
        entityId: categoryId,
        description: `Updated registration category: ${category.name}`,
        adminId,
        adminEmail,
      });
    }
    ...
    console.error('Failed to update registration category:', error);
    throw error;
  }
}

/**
 * Deletes a registration category
 *
 ...
 * @returns {Promise<void>}
 */
export async function deleteRegistrationCategory(categoryId, adminId = null, adminEmail = null) {
  try {
    const categoryRef = doc(db, COLLECTIONS.CONFERENCES, SETTINGS_DOC_ID, 'registrationCategories', categoryId);
    ...
    if (adminId && adminEmail) {
      await logActivity({
        ...
        entityType: ENTITY_TYPES.CATEGORY,
        entityId: categoryId,
        description: `Deleted registration category`,
        adminId,
        adminEmail,
      });
    }
  } catch (error) {
    console.error('Failed to delete registration category:', error);
    throw error;
  }
}
